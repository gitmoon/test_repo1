...
class CommonRegex:
    LINUX_KERNEL_VERSION = re.compile(r"\d\.\d\.\d\d-Welbilt\+\w+")
    ...
    UBOOT_ENV_NOT_DEFINED = re.compile(r"not defined$")
    UBOOT_ENV_PRINT_RESULT = re.compile(r"\w+=\w+")
    UBOOT_SAVEENV_DONE = re.compile(r"done$")
    ...
    UDHCPC_STARTED = re.compile(r"udhcpc \(.+\) started")
    UDHCPC_LEASE_OBTAINED = re.compile(r"Lease of \d+\.\d+\.\d+\.\d+ obtained")
    ...
    IFCONFIG_IP_BCAST_MASK = re.compile(r"inet addr:.+  Bcast:.+  Mask:.+")
    ...
    IFCONFIG_HWADDR = re.compile(r"HWaddr \w{2}:\w{2}:\w{2}:\w{2}:\w{2}:\w{2}")
    ...
    DMESG_RESULT_RTC = re.compile(r"\[.*\] pcf85363 2-0051: rtc core: registered pcf85363 as rtc0")
