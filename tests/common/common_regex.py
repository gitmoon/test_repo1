...
class CommonRegex:
    LINUX_KERNEL_VERSION = re.compile(r"\d\.\d\d\.\d\d-linux4sam-\d\d\d\d\.\d")
    ...
    UBOOT_IP_ADDR_ERROR = re.compile(r"^... ERROR: `ipaddr' not set")
    UBOOT_PING_FAILED = re.compile(r"^ping failed; host .+ is not alive")
    UBOOT_PING_SUCCESS = re.compile(r"^host .+ is alive")
    UBOOT_CURRENT_I2C_DEV = re.compile(r"^Current bus is 1")
    UBOOT_I2C_BUS_READ = re.compile(r"^\d\d\d\d: \w\w \w\w")
    UBOOT_I2C_BUS_READ_AA = re.compile(r"^\d\d\d\d: aa \w\w")
    UBOOT_I2C_BUS_READ_BB = re.compile(r"^\d\d\d\d: bb \w\w")
    ...
    NC_ZV_RESULT = re.compile(f"{TEST_HOST_IP_ADDR} .{TEST_HOST_IP_ADDR}:{NETWORKING_TCP_PORT}. open")
    ...
    UDHCPC_LEASE_OBTAINED = re.compile(r"lease of \d+\.\d+\.\d+\.\d+ obtained, lease time 86400")
    ...
    MAC_LOCAL = re.compile(r"\w[36ae] \w{2} \w{2} \w{2} \w{2} \w{2}")
    ...
    WILL_STORE_MAC_DOTTED = re.compile(r"Ethernet MAC: (\w{2}:\w{2}:\w{2}:\w{2}:\w{2}:\w{2})")
    ...
    IW_POWER_1DBM = re.compile("txpower 1.00 dBm")
    IW_POWER_15DBM = re.compile("txpower 15.00 dBm")
    IW_POWER_20DBM = re.compile("txpower 20.00 dBm")
    WIFI_CHANNEL_LIST = re.compile(r"Channel \d+ : .+ GHz")
    WIFI_COUNTRY_GB = re.compile("country GB")
    WIFI_COUNTRY_US = re.compile("country US")
    LSMOD_BTWILINK = re.compile(r".    \d\.\d\d\d\d\d\d. Bluetooth. hci0. QCA setup on UART is completed")
    DMESG_RESULT_RTC = re.compile(r"\[.*\] at91_rtc f80480b0.rtc: registered as rtc0")
    DMESG_RESULT_TAS_AUDIO = re.compile(r"\[.*\].*TAS5720")
    ...
    BOARD_ID_ON_BOOT = re.compile("Board ID:")
